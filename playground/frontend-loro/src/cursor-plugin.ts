...
import { Decoration, type DecorationAttrs, DecorationSet, EditorView } from "prosemirror-view";

import { CursorEphemeralStore, cursorEq } from "./ephemeralStore";
import {
  CHILDREN_KEY,
  loroSyncPluginKey,
  type LoroDocType,
...
const loroCursorPluginKey = new PluginKey<{ awarenessUpdated: boolean }>("loro-cursor");
...
export const LoroCursorPlugin = (
  ...
  const createCursor =
    ...
    ((user) => {
      const awarenessData = store.getAll();
  ...
  const plugin: Plugin<DecorationSet> = new Plugin<DecorationSet>({
    ...
    state: {
      ...
      apply(tr, prevState, _oldState, newState) {
        ...
        const loroCursorState: { awarenessUpdated: boolean } = tr.getMeta(loroCursorPluginKey);
        if (
          (loroState && loroState.changedBy !== "local") ||
          (loroCursorState && loroCursorState.awarenessUpdated)
        ) {
          return createDecorations(newState, store, plugin, createSelection, createCursor);
        }
    ...
    view: (view) => {
      const storeLocalListener = (event: EphemeralStoreEvent) => {
        if (event.by !== "local") {
          setTimeout(() => {
      ...
      const unsub = store.subscribe(storeLocalListener);
...
function absolutePositionToCursor(
  ...
  const loroId =
    WEAK_NODE_TO_LORO_CONTAINER_MAPPING.get(nodeParent) ?? getByValue(mapping, nodeParent);
  if (!loroId) {
    if (anchor > 1) {
